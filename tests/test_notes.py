...
async def test_create_note(async_client):
    response = await async_client.post("/notes/", json={"title": "Test", "content": "Test content"})
...
async def test_read_notes(async_client):
    await async_client.post("/notes/", json={"title": "Note 1", "content": "Content 1"})
    await async_client.post("/notes/", json={"title": "Note 2", "content": "Content 2"})
...
async def test_update_existing_note(async_client):
    res = await async_client.post("/notes/", json={"title": "Old", "content": "Content"})
    note_id = res.json()["id"]

    update = await async_client.put(f"/notes/{note_id}", json={"title": "Updated", "content": "New content"})
...
async def test_update_nonexistent_note(async_client):
    response = await async_client.put("/notes/9999", json={"title": "New", "content": "Updated"})
...
async def test_delete_existing_note(async_client):
    res = await async_client.post("/notes/", json={"title": "Temp", "content": "To delete"})
    note_id = res.json()["id"]
...
async def test_delete_note_twice(async_client):
    res = await async_client.post("/notes/", json={"title": "Temp", "content": "Temp"})
    note_id = res.json()["id"]
